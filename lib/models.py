...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, xazabd):
        golist = xazabd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_xazabd(xazabd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_xazabd(self, xazabd, rec):
        import decimal
        import xazablib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from xazabd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from xazabd, with every run
        ...
        # get/create, then sync payment amounts, etc. from xazabd - Xazabd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(xazabd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from xazabd! %s" % e)
            govobj.vote_delete(xazabd)
            return (govobj, None)
    ...
    def vote_delete(self, xazabd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(xazabd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, xazabd, signal, outcome):
        import xazablib
        ...
        output = xazabd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = xazablib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(xazabd, signal)

    def sync_network_vote(self, xazabd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = xazabd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = XAZABD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 xazab addr, non-multisig
            if not xazablib.is_valid_xazab_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Xazab address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Xazab Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (xazablib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = XAZABD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not xazablib.is_valid_xazab_address(addr, config.network):
    ...
    def hash(self):
        import xazablib
        return xazablib.hashit(self.serialise())
