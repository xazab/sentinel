...
class XazabDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some xazabd methods
    ...
    @classmethod
    def from_xazab_conf(self, xazab_dot_conf):
        from xazab_config import XazabConfig
        config_text = XazabConfig.slurp_config_file(xazab_dot_conf)
        creds = XazabConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = xazablib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = xazablib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = xazablib.blocks_to_seconds(diff)
