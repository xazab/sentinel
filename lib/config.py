...
sentinel_cfg = XazabConfig.tokenize(sentinel_config_file)
sentinel_version = "1.5.0"


def get_xazab_conf():
    if sys.platform == 'win32':
        xazab_conf = os.path.join(os.getenv('APPDATA'), "XazabCore/xazab.conf")
    else:
        home = os.environ.get('HOME')

        xazab_conf = os.path.join(home, ".xazabcore/xazab.conf")
        if sys.platform == 'darwin':
            xazab_conf = os.path.join(home, "Library/Application Support/XazabCore/xazab.conf")

    xazab_conf = sentinel_cfg.get('xazab_conf', xazab_conf)
