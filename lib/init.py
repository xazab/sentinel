...
def has_xazab_conf():
    ...
    valid_xazab_conf = False

    # ensure xazab_conf exists & readable
    #
    # if not, print a message stating that Xazab Core must be installed and
    # configured, including JSONRPC access in xazab.conf
    try:
        f = io.open(config.xazab_conf)
        valid_xazab_conf = True
    except IOError as e:
        print(e)

    return valid_xazab_conf
...
def main():
    ...
    if not has_xazab_conf():
        print("XazabCore must be installed and configured, including JSONRPC access in xazab.conf")
        sys.exit(1)
